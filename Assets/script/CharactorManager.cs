		...
		void Start ()
		{
			...
			for (int i=0; i<2; i++) {
				for (int j=0; j<5; j++) {
					...
					((armature.Display as UnityArmatureDisplay).Display as GameObject).transform.position = new Vector3((float)j+r0*2f, 1, 0);
			...
			for (int i=0; i<2; i++) {
				...
				float r2 = (float)random.NextDouble();
				((armature.Display as UnityArmatureDisplay).Display as GameObject).transform.position = new Vector3((float)j+r0*2f, (float)i*3f+r1*5f, 0);
				WorldClock.Clock.Add (armature);
